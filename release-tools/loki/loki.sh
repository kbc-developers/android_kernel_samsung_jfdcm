...
egrep -q -f /tmp/loki/loki_bootloaders /proc/cmdline
if [ $? -eq 0 ];then
  mkdir -p $C
  dd if=/dev/block/platform/msm_sdcc.1/by-name/aboot of=$C/aboot.img
  /tmp/loki/loki_patch boot $C/aboot.img /tmp/boot.img $C/boot.lok || exit 1
  /tmp/loki/loki_flash boot $C/boot.lok || exit 1
  rm -rf $C
  exit 0
fi

egrep -q -f /tmp/loki/unlocked_bootloaders /proc/cmdline
if [ $? -eq 0 ];then
